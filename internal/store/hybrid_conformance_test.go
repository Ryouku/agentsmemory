...
// TestHybridRunsTheConformanceSuite pairs the two REAL backends production
// pairs, rather than two fakes.
//
// Hybrid is the implementation every deployment actually holds, and it is the
// one whose behaviour is not its own: it forwards, and a forward to the wrong
// half is invisible to a test of either half alone.
func TestHybridRunsTheConformanceSuite(t *testing.T) {
	storetest.RunPointsConformance(t, "hybrid", func(t *testing.T) store.VectorStore {
		dir := t.TempDir()
