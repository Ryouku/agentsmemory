...
func TestRerankBlendsRatherThanOverwrites(t *testing.T) {
	...
	blended, _ := svc.applyRerankWith(context.Background(), "q", "q", survivors, ranked, DefaultRerankWeight)
	...
	if over, _ := svc.applyRerankWith(context.Background(), "q", "q", survivors, ranked, 1); survivors[over[0].Index].Drawer.ID != "B" {
		t.Error("w=1 must hand the decision to the cross-encoder")
	}
	// w=0 does not consult it at all.
	if none, _ := svc.applyRerankWith(context.Background(), "q", "q", survivors, ranked, 0); survivors[none[0].Index].Drawer.ID != "A" {
...
func TestRerankKeepsTheWholePage(t *testing.T) {
	...
	svc.rerankPool = 3
	if got, _ := svc.applyRerankWith(context.Background(), "q", "q", survivors, ranked, DefaultRerankWeight); len(got) != 3 {
