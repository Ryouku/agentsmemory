...
⚠ **AMENDED 2026-08-28, after the questions above were resolved: COST WAS NEVER THE BINDING CONSTRAINT, and this section's framing said it was.** Measured the same evening in output tokens, which is the currency an agent actually spends: an `am_search` emission costs ~30 output tokens, an `am_get_drawer` ~45, a content-bearing drawer write ~400, and a diary entry ~525. A read is one of the cheapest actions available; the mandatory write side is roughly **10×** the entire read-side protocol. And in the very session that produced this amendment — under an explicit instruction to recall more — the ratio measured **6 searches against 18 writes, 3.0 writes per read**, worse than the 1.9 in M-2. Reads are not rare because they are expensive.
...
| 13 | What quantity does F-5's counting rule count? | F-5 | Reads ACTED ON WITHOUT A SECOND CALL. Fixed 2026-08-28 before collection, which is F-5's own requirement and the only moment it is cheap — F-6 makes every baseline taken under a superseded rule void. Read FREQUENCY was the obvious choice and is rejected on measurement: reads cost ~30 output tokens against ~400-525 for a write, so price is not why they are rare, and the same evening measured 3.0 writes per read under an explicit instruction to read more. A frequency rule would repeat ADR-041's failure in a new location — counting the quantity that is easy to count rather than the one being claimed |
