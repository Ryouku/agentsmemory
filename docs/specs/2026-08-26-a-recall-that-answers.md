...
| F-6 | The fact-retrieval arm scores ordering by MRR on the same paired bootstrap as every other arm, so a fact ranked second is distinguishable from one ranked tenth. This is a property of the INSTRUMENT and is measurable before facts reach the page — the arm scores the ranked fact list it retrieves, whatever consumes it | `internal/palace/recallanswers_spec_test.go::TestFactsOnThePageAreScoredByMRR` | @spec | |
