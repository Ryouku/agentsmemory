...
|------|---------------|----------|-------|
| `internal/mcptest` harness | a running server | real HTTP transport, real MCP client, real registration; the hosted OAuth gate is stubbed for ordinary scenarios, while local-only scenarios cross the real `auth.LocalTenant` edge | `internal/mcptest/harness_test.go` `TestHarnessObservesAWriteThroughARead`, `TestHarnessFailsOnAnEmptyCatalogue`; lifecycle registry uses `NewLocalWithWing` for `am_delete_wing` |
