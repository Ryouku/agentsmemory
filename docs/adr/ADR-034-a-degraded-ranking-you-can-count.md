...
(min 7.3s, max 18.2s). A second run at the same pool on the same hardware averaged roughly 17s with
calls reaching 19.7s. `docker-compose.full.yml` ships `RERANK_TIMEOUT: "10s"`, so a majority of
...
`search_events.reranked` is `boolToInt(reranked)` (`internal/palace/service.go:1182`), where that
