...
4. Extract the arms-list construction from `EvaluateWith` into `evalArms(opts EvalOptions, rerank bool) []EvalArm` and call it from both `EvaluateWith` and the test. `TestEveryDeclaredArmIsRegistered` (`armreach_test.go`) parses the function that assembles the list, so keep the appends inside the extracted function or that gate stops seeing them.
