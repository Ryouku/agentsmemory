...
- **T3 — record the fetch against the recall and report the ratio** (deferred: `docs/adr/BACKLOG.md` §"From ADR-028" — trigger: the first week `am_get_drawer` receives a non-empty `search_id` from a non-test client)
- **`profile_id` on the durable `search_events` row** (deferred: `docs/adr/BACKLOG.md` §"From ADR-028" — BACKLOG primitive 1 is on the span already; putting it on the row is a migration and belongs with T3, since a ratio is uninterpretable without knowing which profile produced it)
