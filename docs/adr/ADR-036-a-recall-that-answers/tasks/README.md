...
|----|-------|--------|--------|------------|
| T1 | The instrument: a fact answerable-rate with a 0% baseline | pending | F-5, F-6 | 2 tests, frozen corpus + suite |
| T2 | A lookup that distinguishes four outcomes, not two | pending | F-12 | 2 tests (palace + mcpserver) + suite |
...
| T7 | A wing reports its own entry point, resolved directly | pending | F-10, F-17, UC4-S1, UC4-S2 | 3 tests (2 palace + catalogue) + suite |
| T4 | Both entity vocabularies, and an ended fact is never current | pending | F-4, F-7, UC1-S2 | 2 tests + suite |
| T5 | A corrected record arrives carrying its correction | pending | F-3, UC3-S1, UC3-S2 | 2 tests (palace + mcpserver) + suite |
| T8 | The protocol becomes an API, and proves it costs less for the same meaning | pending | F-13, F-14, F-15, F-16, F-19, UC6-S1, UC6-S2, UC6-S3 | 6 tests (5 palace + catalogue) + suite |
...
- **`-skip`** — all 26 ADR-036 stubs are committed failing (20 in `internal/palace`, 6 in
  `internal/mcpserver`, verified 2026-08-26), so an unskipped `go test ./...` stays red until the
  ...
  depend on, so a fence still runs its ancestors' tests: T1 2 · T2 2 · T6 2 · T7 5 · T3 11 · T4 13 ·
  T5 13 · T8 24.

Proven two-sided 2026-08-26: T1's fence exits 1 today and exits 0 with only T1's own stubs
neutralised, while T3's fence still exits 1 in that same state. **No single fence runs all 26** — T8
...
**Six of the 26 stubs live in `internal/mcpserver`, and that is deliberate.** No test in
...
These sit beside `catalog_test.go` and `hitview_test.go`, which exist for the same reason.
...
| T2 | `kg.Resolution` (four-state lookup) | T3, T7 | T2 before T3 — the pointer rests on it; T7 reuses its absence vocabulary |
