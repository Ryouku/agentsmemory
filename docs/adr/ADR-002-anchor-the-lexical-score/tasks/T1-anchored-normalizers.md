...
1. Write the failing tests first (TDD red), in `internal/palace/rank_test.go`:
   ...
   - `TestLexNormBoostHasNoEquivalentPageMaxWeight` — on one hand-built fixture with a nonzero closet boost on some candidates, no `w'` on a fine grid over `[0,1]` reproduces the `ceiling` ordering at `w`. Asserted as a grid search rather than as "differs at `w'`" so it is deterministic and states the real claim.
...
8. Run the acceptance command; the whole existing rank suite must be green alongside the new tests.
...
| `TestLexNormBoostHasNoEquivalentPageMaxWeight` | `internal/palace/rank_test.go` | with an additive closet boost, no page-max weight on a grid over `[0,1]` reproduces the anchored ordering | — |
