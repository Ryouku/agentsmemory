...
- [x] **CLOSED 2026-08-22 — received from ADR-018, and the premise it rested on is gone.** The question was whether a subagent's recalls should carry its own identity or its dispatcher's. It assumed ADR-018 would add a session identity to every recall; **ADR-018 T2 was WITHDRAWN** in favour of keeping the transport stateless, so the server records no identity for anyone and there is nothing to attribute with. Closed rather than deferred: it is not waiting on effort, it is waiting on a decision that was taken the other way.

  One fact worth keeping for whoever reopens it. The HARNESS already distinguishes them, even though the server does not: a captured `SubagentStop` payload (T3, in `hooks_test.go`) carries **`agent_id`, distinct per subagent**, alongside a `session_id` **identical to the parent's**. So if attribution is ever wanted, the identity exists at the client and the missing piece is a client that sends it — which is exactly the option ADR-018 rejected, for reasons that would apply again.
