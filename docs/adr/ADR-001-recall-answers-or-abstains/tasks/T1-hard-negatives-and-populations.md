...
4. Widen `verifyAbsent` from `Limit: 3` to `absentVerifyDepth = 20`, with the reason at the constant: the eval's retrieval ceiling on this corpus puts 98% of answerable golds inside the top 20 by vector distance (top-1 75%, top-5 92%, top-20 98%, 1 of 40 never retrieved), so depth 20 checks everything this palace can retrieve at all. It is not a corpus-wide proof and the constant's comment must say so — a memory the dense channel never surfaces is one recall could not have returned either.
