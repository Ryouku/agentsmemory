...
- **CLOSED 2026-08-21 (`d150b7e`) — two more producers, found by running this ADR's question against every path that writes a drawer's content.** T2 fixed `Add` and T4 fixed `WriteDiary`; enumerating the siblings found `Service.Update` and `AbsorbDrawers` as well.
  - `Update` re-embedded the new content and left the PREVIOUS content's entities on the row. Reproduced before fixing: a memory filed as "Redis beat Postgres", updated to say "Kafka beat Mongo", stored `[Postgres Redis]`. This is worse in kind than the defect this ADR named — the other three produce a memory MISSING from the derived graph, this one produces a graph asserting an edge the text does not support. An empty graph sends an agent to go and look; a wrong one tells it not to. Fixed in `Repo.Update` rather than in the service, so content and its derived entities are written by the same statement and a future call site cannot write one without the other.
  - `AbsorbDrawers` took entities from the import record. **That is a deliberate semantics change and it is recorded here rather than only in a commit message.** Replaying the source's entities verbatim is right when the source HAS them — this is a migration, and re-deriving would overwrite another palace's extraction with this build's — but a source supplying none filed every memory permanently outside the graph, because `RecomputeGraph` reads the column and never re-extracts. Not hypothetical: an export from any palace predating this ADR carries no entities, and this palace held 0 of 359 the day before it landed. Entities are now derived ONLY into that gap, so the verbatim contract holds unchanged for every record that has one; `TestAbsorbDerivesEntitiesOnlyWhenSourceGaveNone` pins both halves, because each half is a way to get it wrong.
