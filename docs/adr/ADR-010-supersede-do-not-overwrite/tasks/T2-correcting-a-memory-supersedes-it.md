# Task ADR-010-T2: Correcting a memory supersedes it, and erasure leaves the agent surface

**Depends-on:** T1
**Covers:** none — no spec
**Estimated scope:** M (multi-file)
**Owner:** unassigned
**Produces:** supersede semantics on `am_update_drawer`; erasure moved to the operator surface
...
| `internal/mcpserver/drawers.go` | edit | `am_update_drawer` returns the new id and names the ended one; `am_delete_drawer` deregistered from the agent catalogue |
...
4. Deregister `am_delete_drawer`. `TestCatalogSizeIsWhatTheReadmeClaims` will fail — update the README count in the same commit, since that gate exists to make exactly this visible.
...
| `TestAgentSurfaceHasNoErasure` | `internal/mcpserver/catalog_test.go` | no agent-facing tool hard-deletes a memory | — |
