...
| [T2](T2-entities-on-the-write-path.md) | A drawer an agent files carries its entities | populated `drawers.entities` | T1 | done — `Add` stamps entities per chunk, and the lexicon is fixed first: 47/163 ordinary words surviving → 2, every acronym kept |
| [T3](T3-an-empty-graph-says-why.md) | A graph tool that cannot answer says so | `emptyGraphNote` | none | done |
| [T4](T4-a-diary-entry-is-a-memory-too.md) | A diary entry is a memory too | `entities` on the diary path | T2 | done |
